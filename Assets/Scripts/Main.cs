...
public class Main : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        TextAsset textAsset = Resources.Load("white") as TextAsset;
        ...
        for(int i = 0;i < measureNum;i++)
        {
           ...
           foreach (LineModel lm in lines)
               if (lm.measure == i) 
               {
                   Debug.Log(lm.measure); // maybe beat?
                   int split = lm.data.Length / 2;

                   int pos = 0;
                   Debug.Log(lm.data); // actual score data
                   foreach (string n in lm.data.SubstringAtCount(2)) // cut by 2 and process by 2
                   {
